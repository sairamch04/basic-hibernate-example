...
import com.home.hibernate.entity.A;
import com.home.hibernate.entity.Address;
import com.home.hibernate.entity.B;
...
public class HibernateTest {
    ...
    @Test
    public void testMineDelete() {
        Session session = sessionFactory.openSession();
        session.beginTransaction();
        A a = session.load(A.class, 8);
        //removing 1st element in the list. This just removes the relationsip b/w A and B.i.e it sets the FK reference to null
        a.getBList().remove(0);
    ...
    public void testEagerLoading() {
        System.out.println(
                "testEagerLoading begins ......Prequsite is change the fetch type to eager for insurances variable in Person class. @ElementCollection(fetch=FetchType.EAGER)");
        testElementCollection();
        Session session = sessionFactory.openSession();
        session.beginTransaction();
        Person p = session.get(Person.class, 1L);
        ...
        session.close();
        System.out.println(
                "Test case is passed if there is no LazyInitializationException occurs, if there is no expception next line will have the list of companies");
        System.out.println(p.getInsurances());
    ...
    public void testSingleTableStrategy() {
        System.out.println(
                "testSingleTableStrategy begins. Uncomment @Inheritance(strategy=InheritanceType.SINGLE_TABLE) in Vehicle class");
        Session session = sessionFactory.openSession();
        session.beginTransaction();
    ...
    public void testTablePerClassStrategy() {
        System.out.println(
                "testTablePerClassStrategy begins. Uncomment @Inheritance(strategy=InheritanceType.TABLE_PER_CLASS) in Vehicle class ");
        Session session = sessionFactory.openSession();
        session.beginTransaction();
    ...
    public void testTableJoinedStrategy() {
        System.out.println(
                "testTableJoinedStrategy begins. Uncomment @Inheritance(strategy=InheritanceType.JOINED) in Vehicle class ");
        Session session = sessionFactory.openSession();
        session.beginTransaction();
    ...
    public void testTransientObject() {
        ...
        session.getTransaction().commit();
        System.out.println(
                "Deleted the person, so retrieve one more time to confirm. After deleteing the object becomes transient");
    ...
    public void testPersistingDetachedObject() {
        ...
        session.close();
        System.out.println(
                "Saved the object and closed the session and now updating using setter method. It's detached object now");
        person.setFirstName("updated the name");
        System.out.println("The upadted object after saving and closing the session is " + person);
        session = sessionFactory.openSession();
        session.beginTransaction();
